...
def uploaded(uploaded_file, session):
    ...
    # 후처리(conf_threshold=0.35로 설정)
    boxes = yolov8_postprocess(output, conf_threshold=0.35 ,debug=False)

    detected_items = extract_detected_items(boxes, class_names, conf_threshold=0.35)  # B; 결과에서 품목 추출

    boxes_scaled = scale_boxes(boxes, input_shape=(640, 640), orig_shape=img.shape[:2])
    result_img = draw_boxes(img, boxes_scaled, class_names, conf_threshold=0.35) # B; 이미지 위에 박스그릠
...
if uploaded_file:
    ...
    st.markdown(f"**{", ".join(detected_items)}**")
